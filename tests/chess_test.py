...
def test_legal_knight_moves():
    ...
    with check:
        ...
        assert board.get_legal_moves(index) == unordered(['d6-c4', 'd6-e4', 'd6-f5', 'd6-f7',
                                                          'd6-c8', 'd6xe8', 'd6-b5', 'd6-b7'])
...
def test_legal_rook_moves():
    ...
    with check:
        assert board.get_legal_moves(index) == unordered(['b6-b7', 'b6-b8', 'b6-b5', 'b6-b4',
                                                          'b6-b3', 'b6-b2', 'b6-b1', 'b6-a6',
                                                          'b6-c6', 'b6-d6', 'b6-e6', 'b6-f6',
                                                          'b6-g6', 'b6-h6'])
...
def test_legal_bishop_moves():
    ...
    with check:
        assert board.get_legal_moves(index) == unordered(['e4-d5', 'e4-c6', 'e4-b7', 'e4-a8',
                                                          'e4-d3', 'e4-c2', 'e4-b1', 'e4-f5',
                                                          'e4-g6', 'e4-h7', 'e4-f3', 'e4-g2',
                                                          'e4-h1'])
    index = chess.algebraic_to_board_index('d7')
    with check:
        assert board.get_legal_moves(index) == unordered(['d7-c8', 'd7-c6', 'd7-b5', 'd7-a4',
                                                          'd7-e6', 'd7-f5', 'd7xg4'])
...
def test_promotion():
    ...
    with check:
        index = chess.algebraic_to_board_index('b2')
        assert board.get_legal_moves(index) == unordered(['b2-b1=q', 'b2-b1=r', 'b2-b1=b', 'b2-b1=n',
                                                      'b2xc1=q', 'b2xc1=r', 'b2xc1=b', 'b2xc1=n'])

    board.set_fen('1b2k3/2P5/8/8/8/8/8/4K3 w - - 0 1')
    with check:
        index = chess.algebraic_to_board_index('c7')
        assert board.get_legal_moves(index) == unordered(['c7-c8=Q', 'c7-c8=R', 'c7-c8=B', 'c7-c8=N',
                                                          'c7xb8=Q', 'c7xb8=R', 'c7xb8=B', 'c7xb8=N'])
    with check:
